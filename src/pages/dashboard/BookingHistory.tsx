...
export default function BookingHistory() {
  ...
  const generateReceipt = async (booking: any) => {
    ...
    try {
      ...
      const vehicle = booking.vehicle ? `${booking.vehicle.make} ${booking.vehicle.model}${booking.vehicle.year ? ` ${booking.vehicle.year}` : ''} (${booking.vehicle.registration_number})` : 'Unknown Vehicle';
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        ...
        <p className="text-slate-400 mt-1">View your past charging sessions.</p>
      </div>
                ...
                {isLoading ? (
                  [...Array(5)].map((_, index) => (
                    <tr key={index} className="bg-slate-900 border-b border-slate-800">
                      <td className="px-6 py-4"><div className="h-4 bg-slate-800 rounded animate-pulse w-32"></div></td>
                      <td className="px-6 py-4"><div className="h-4 bg-slate-800 rounded animate-pulse w-48"></div></td>
                      <td className="px-6 py-4"><div className="h-4 bg-slate-800 rounded animate-pulse w-24"></div></td>
                      <td className="px-6 py-4"><div className="h-4 bg-slate-800 rounded animate-pulse w-16"></div></td>
                      <td className="px-6 py-4"><div className="h-6 bg-slate-800 rounded-full animate-pulse w-20"></div></td>
                      <td className="px-6 py-4"><div className="h-4 bg-slate-800 rounded animate-pulse w-16"></div></td>
                    </tr>
                  ))
                ) : bookings.length > 0 ? (
                  bookings.map((booking) => (
                    <tr key={booking.id} className="bg-slate-900 border-b border-slate-800 hover:bg-slate-50 hover:bg-slate-800/50">
                      ...
                      <td className="px-6 py-4">
                        {booking.vehicle ? (
                          <>
                            ...
                            {booking.vehicle.year && <span className="text-slate-500 text-xs ml-1">({booking.vehicle.year})</span>}
                            <div className="text-xs text-slate-500 mt-0.5">{booking.vehicle.registration_number}</div>
                          </>
                        ) : 'Unknown Vehicle'}
                      </td>
                      <td className="px-6 py-4">₹{Number(booking.estimated_price).toFixed(2)}</td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center justify-center whitespace-nowrap w-max px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          booking.status === 'completed' || booking.status === 'paid' ? 'bg-emerald-900/30 text-emerald-400' :
                          booking.status === 'pending' || booking.status === 'evaluating' ? 'bg-amber-900/30 text-amber-400' :
                          ['assigned', 'enroute', 'arrived', 'charging'].includes(booking.status) ? 'bg-blue-900/30 text-blue-400' :
                          booking.status === 'pending_payment' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' :
                          'bg-red-900/30 text-red-400'
                        }`}>
                          {(booking.status === 'completed' || booking.status === 'paid') && <CheckCircle2 className="w-3 h-3 mr-1" />}
                          {(booking.status === 'pending' || booking.status === 'evaluating') && <Clock className="w-3 h-3 mr-1" />}
                          {booking.status === 'cancelled' && <XCircle className="w-3 h-3 mr-1" />}
                          {booking.status === 'pending' ? 'Searching' : 
                           booking.status === 'pending_payment' ? 'Awaiting Payment' :
                           booking.status === 'enroute' ? 'En Route' :
                           booking.status === 'assigned' ? 'Van Assigned' :
                           booking.status === 'charging' ? 'Charging in Progress' :
                      ...
                      <td className="px-6 py-4">
                        {booking.status === 'pending_payment' ? (
                          <div className="flex gap-3">
                            <Link 
                              ...
                              state={{ bookingId: booking.id, amount: booking.estimated_price, vehicleType: booking.vehicle?.vehicle_type }}
                              className="text-blue-400 font-medium hover:underline"
                            >
                              Pay Now
                            </Link>
                          </div>
                        ) : booking.status === 'pending' || booking.status === 'evaluating' ? (
                           <div className="flex gap-3">
                             <button onClick={() => handleCancelBooking(booking.id)} className="text-red-400 hover:underline">Cancel</button>
                           </div>
                        ...
                        ) : booking.status === 'cancelled' || booking.status === 'failed' || booking.status === 'rejected' ? (
                          <span className="text-slate-400 italic capitalize">{booking.status === 'rejected' ? 'Failed' : booking.status}</span>
                        ) : (
      ...
      {cancellingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-slate-900 p-6 rounded-xl shadow-xl max-w-md w-full mx-4 border border-slate-800">
